...
with col_chat_title:
    ...
    # 레벨 선택 버튼 스타일 커스터마이징 - 사각형 모양으로 변경
    st.markdown("""
    <style>
...
with col_news_title:
    ...
    # 모드 버튼 스타일 - 주식버튼 스타일로 통일
    st.markdown("""
    <style>
    /* 뉴스/주식 모드 버튼 - 주식버튼 스타일 기준으로 통일 */
    button[key="mode_news"], 
    button[key="mode_stock"],
    button[data-testid="stBaseButton-primary"][key="mode_news"],
    button[data-testid="stBaseButton-secondary"][key="mode_news"],
    button[data-testid="stBaseButton-primary"][key="mode_stock"],
    button[data-testid="stBaseButton-secondary"][key="mode_stock"] {
    ...
    /* 활성 상태 스타일 */
    button[key="mode_news"][kind="primary"],
    button[key="mode_stock"][kind="primary"],
    button[data-testid="stBaseButton-primary"][key="mode_news"],
    button[data-testid="stBaseButton-primary"][key="mode_stock"] {
        background-color: rgb(0, 51, 153) !important;
        color: white !important;
    ...
    /* 호버 효과 */
    button[key="mode_news"]:hover, 
    button[key="mode_stock"]:hover,
    button[data-testid="stBaseButton-primary"][key="mode_news"]:hover,
    button[data-testid="stBaseButton-secondary"][key="mode_news"]:hover,
    button[data-testid="stBaseButton-primary"][key="mode_stock"]:hover,
    button[data-testid="stBaseButton-secondary"][key="mode_stock"]:hover {
        transform: scale(1.05) !important;
        background-color: rgba(0, 51, 153, 0.8) !important;
        color: white !important;
    }
...
with col_news:
    ...
    else:  # 주식 모드 - 전체 UI 톤에 맞게 개선
        # 주식 정보 컨테이너
        with st.container(height=450):
            # 주식 검색 결과 표시
            if st.session_state.stock_query:
                company_name = st.session_state.stock_query.strip()
                ...
                if company_name:
                    ...
                    if stock_result["success"]:
                        ...
                        <div class="stock-header">
                            <div style="font-weight: 600; font-size: 14px; color: #202124;">📊 {company_name}</div>
                            <div style="font-size: 11px; color: #5f6368; margin-top: 2px;">종목코드: {stock_result['code']}</div>
                        </div>
                        """, unsafe_allow_html=True)
                        ...
                        if not stock_data.empty and len(stock_data) > 0:
                            try:
                                ...
                                # 주가 상태에 따른 스타일링
                                if price_change > 0:
                                    price_class = "stock-price-up"
                                    color = "#ea4335"
                                    arrow = "📈"
                                    trend = "상승"
                                elif price_change < 0:
                                    price_class = "stock-price-down"
                                    color = "#1a73e8"
                                    arrow = "📉"
                                    trend = "하락"
                                else:
                                    price_class = "stock-price-neutral"
                                    color = "#5f6368"
                                    arrow = "📊"
                                    trend = "보합"
                                ...
                                # 메인 주가 카드
                                st.markdown(f"""
                                <div class="stock-card stock-price-main {price_class}">
                                    <div style="display: flex; justify-content: space-between; align-items: center;">
                                        <div>
                                            <div style="font-size: 18px; font-weight: 600; color: #202124; margin-bottom: 4px;">
                                                {latest_price:,.0f}원
                                            </div>
                                        ...
                                        <div style="text-align: center;">
                                            <div style="font-size: 20px; margin-bottom: 2px;">{arrow}</div>
                                            <div style="font-size: 10px; color: {color}; font-weight: 500;">{trend}</div>
                                        </div>
                                    </div>
                                </div>
                                """, unsafe_allow_html=True)
                                ...
                                try:
                                    ...
                                    <div class="stock-info-grid">
                                        <div class="stock-info-item">
                                            <div style="font-size: 10px; color: #5f6368; margin-bottom: 2px;">당일 최고가</div>
                                            <div style="font-size: 13px; font-weight: 600; color: #ea4335;">{high_price:,.0f}원</div>
                                        </div>
                                        <div class="stock-info-item">
                                            <div style="font-size: 10px; color: #5f6368; margin-bottom: 2px;">당일 최저가</div>
                                            <div style="font-size: 13px; font-weight: 600; color: #1a73e8;">{low_price:,.0f}원</div>
                                        </div>
                                    </div>
                                    """, unsafe_allow_html=True)
                                except:
                                    st.markdown("""
                                    <div class="stock-info-item" style="text-align: center; color: #5f6368; font-size: 11px;">
                                        고가/저가 정보를 표시할 수 없습니다
                                    </div>
                                    """, unsafe_allow_html=True)
                                ...
                                if len(stock_data) >= 3:
                                    ...
                                    <div style="margin: 12px 0 8px 0;">
                                        <div style="font-size: 12px; font-weight: 600; color: #202124; margin-bottom: 8px;">📋 최근 거래 현황</div>
                                    </div>
                                    """, unsafe_allow_html=True)
                                ...
                                <div style="text-align: center; margin-top: 12px;">
                                    <span style="font-size: 10px; color: #5f6368;">📡 데이터 제공: {source_text}</span>
                                </div>
                                """, unsafe_allow_html=True)

                            except Exception as e:
                                st.markdown(f"""
                                <div class="stock-no-data">
                                    <div style="font-size: 16px; margin-bottom: 8px;">⚠️</div>
                                    <div style="font-weight: 600; margin-bottom: 4px;">데이터 처리 오류</div>
                                    <div style="font-size: 11px;">주가 데이터 처리 중 오류가 발생했습니다</div>
                                </div>
                                """, unsafe_allow_html=True)
                            ...
                            <div class="stock-no-data">
                                <div style="font-size: 16px; margin-bottom: 8px;">⚠️</div>
                                <div style="font-weight: 600; margin-bottom: 4px;">주가 데이터 없음</div>
                                <div style="font-size: 11px;">거래 정지 종목이거나 최신 데이터가 없을 수 있습니다</div>
                            </div>
                            """, unsafe_allow_html=True)
                    else:
                        st.markdown(f"""
                        <div class="stock-no-data">
                            ...
                            <div style="font-weight: 600; margin-bottom: 4px;">검색 결과 없음</div>
                            <div style="font-size: 11px;">{stock_result['error']}</div>
                        </div>
                        """, unsafe_allow_html=True)
            else:
                # 초기 화면 - UI 톤에 맞춤
                ...
                <div class="stock-empty-state">
                    ...
                    <div style="font-size: 14px; font-weight: 600; color: #202124; margin-bottom: 8px;">실시간 주식 정보</div>
                    <div style="font-size: 11px; color: #5f6368; line-height: 1.4; margin-bottom: 12px;">
                        상장기업 이름을 입력하여<br>
                        최신 주가와 차트를 확인하세요
                    </div>
                    <div style="background: white; padding: 6px 10px; border-radius: 12px; display: inline-block; border: 1px solid #e9ecef;">
                        <span style="font-size: 10px; color: #5f6368;">💡 예시: 삼성전자, LG화학, 카카오</span>
                    </div>
                </div>
                """, unsafe_allow_html=True)

        # 주식 검색창 - 뉴스 검색창과 동일한 스타일
        stock_search = st.chat_input("기업명을 입력하세요")

        if stock_search:
            st.session_state.stock_query = stock_search
            st.rerun()